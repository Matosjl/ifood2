...
export function PainelPedidosRestaurante() {
  ...
  const fetchPedidos = useCallback(async () => {
    ...
    try {
      ...
      // Detecta novos pedidos pendentes → som + impressão automática
      const prevIds = new Set(prevPedidosRef.current.map((p) => p.id));
      const novosPendentes = novos.filter((p) => p.status === "pendente" && !prevIds.has(p.id));
      if (novosPendentes.length > 0 && audioEnabledRef.current) {
        playNotificationSound();
        setNovoPedidoAlert(true);
        setTimeout(() => setNovoPedidoAlert(false), 5000);
        // Imprime cada novo pedido silenciosamente
        novosPendentes.forEach((p) => reimprimirPedido(p));
      }
  ...
  // ── Formata cupom em texto puro para impressora térmica ────────────────────
  const buildCupom = (pedido) => {
    const sep  = "--------------------------------";
    const line = (l, r) => {
      const sp = Math.max(1, 32 - l.length - r.length);
    ...
    const linhas = [
      ...
      sep,
      ...pedido.itens.map((i) =>
        line(`${i.qtd}x ${i.nome}`, `R$${(i.qtd * i.precoUnitario).toFixed(2)}`)
      ),
      sep,
      line("TOTAL", `R$${Number(pedido.total).toFixed(2)}`),
      sep,
    ];
    if (pedido.observacao) linhas.push(`OBS: ${pedido.observacao}`);
    linhas.push("", new Date().toLocaleString("pt-BR"), "");
    return linhas.join("\n");
  };

  // ── Envia para a fila da API (agente Windows imprime silencioso) ───────────
  const reimprimirPedido = async (pedido) => {
    try {
      await axios.post(`${API}/print/queue`, { content: buildCupom(pedido) });
    } catch {
      // fallback: impressão pelo navegador se a API não responder
      const w = window.open("", "_blank");
      w.document.write(`<pre style="font-family:monospace;font-size:11px">${buildCupom(pedido)}</pre>`);
      w.document.close();
